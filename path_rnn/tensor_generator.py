...
def get_indexed_paths(q_relation_paths,
                      ...
                      replace_in_doc=True,
                      truncate_doc=False,
                      relation_token_default=PAD,
                      entity_default=PROTEIN):
    '''
    ...
    :param relation_token_embeddings: Class implementing the abstract class Embeddings
    :param entity_embeddings: Class implementing the abstract class Embeddings
    :param target_relation_embeddings: Class implementing the abstract class Embeddings
    ...
    indexed_relation_paths = np.full(shape=[total_paths,
                                            max_path_length,
                                            max_relation_length],
                                     fill_value=relation_token_embeddings.get_idx(relation_token_default))
    indexed_entity_paths = np.full(shape=[total_paths,
                                          max_path_length],
                                   fill_value=entity_embeddings.get_idx(entity_default))
    ...
    for query_idx, (relation_paths, entity_paths, target_relation) in enumerate(zip(q_relation_paths,
        ...
        for (rel_path, ent_path) in zip(relation_paths,
            ...
            indexed_target_relations[path_idx] = target_relation_embeddings.get_idx(target_relation)

            for rel_idx, (rel, ent_pair) in enumerate(zip(rel_path,
                                                          zip(ent_path, ent_path[1:] + [-1]))):
                indexed_entity_paths[path_idx, rel_idx] = entity_embeddings.get_idx(ent_pair[0])
                if rel != -1:
                    if replace_in_doc:
                ...
                for word_idx, word in enumerate(rel_doc):
                    indexed_relation_paths[path_idx, rel_idx, word_idx] = relation_token_embeddings.get_idx(word)
            path_idx += 1
